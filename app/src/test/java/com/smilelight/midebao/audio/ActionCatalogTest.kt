...
class ActionCatalogTest {
    ...
    fun `catalog has exactly 17 actions`() {
        assertEquals(17, ActionCatalog.allActions.size)
    }

    @Test
    fun `F3 has 7 speed levels`() {
        val f3Actions = ActionCatalog.allActions.filter { it.actionCode == "F3" }
    ...
    fun `F5 has 7 speed levels`() {
        val f5Actions = ActionCatalog.allActions.filter { it.actionCode == "F5" }
    ...
    fun `F4 F6 F7 each have exactly one action`() {
        assertEquals(1, ActionCatalog.allActions.count { it.actionCode == "F4" })
        assertEquals(1, ActionCatalog.allActions.count { it.actionCode == "F6" })
        assertEquals(1, ActionCatalog.allActions.count { it.actionCode == "F7" })
    }

    @Test
    fun `F3 targetEnergy increases monotonically with speed level`() {
        val f3Actions = ActionCatalog.allActions
    ...
    fun `F5 targetEnergy increases monotonically with speed level`() {
        val f5Actions = ActionCatalog.allActions
    ...
    fun `speedableCodes contains F3 and F5`() {
        assertTrue("F3" in ActionCatalog.speedableCodes)
        assertTrue("F5" in ActionCatalog.speedableCodes)
    }

    @Test
    fun `speedableCodes does not contain F4 F6 F7`() {
        assertTrue("F4" !in ActionCatalog.speedableCodes)
        assertTrue("F6" !in ActionCatalog.speedableCodes)
        assertTrue("F7" !in ActionCatalog.speedableCodes)
    }

    @Test
    fun `all targetEnergy values are in range minus 1 to 1`() {
        ActionCatalog.allActions.forEach { action ->
            assertTrue(
                "${action.actionCode} L${action.speedLevel} targetEnergy ${action.targetEnergy} out of range",
                action.targetEnergy in -1.0..1.0
            )
        }
    }

    @Test
    fun `all targetBrightness values are in range minus 1 to 1`() {
        ActionCatalog.allActions.forEach { action ->
            assertTrue(
                "${action.actionCode} L${action.speedLevel} targetBrightness ${action.targetBrightness} out of range",
                action.targetBrightness in -1.0..1.0
            )
        }
    }
