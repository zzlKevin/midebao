...
import com.smilelight.midebao.audio.AudioAnalysisEngine
import com.smilelight.midebao.audio.FrameResult
import com.smilelight.midebao.action.ActionCatalog
import com.smilelight.midebao.action.ActionSelector
import com.smilelight.midebao.action.SwitchDecision
...
class MainActivity : AppCompatActivity() {
    ...
    // ===== 新架构：音频分析引擎 + 动作选择器 =====
    private val audioEngine = AudioAnalysisEngine(sampleRate = 16000, fftSize = 2048)
    private val actionSelector = ActionSelector()
    // 当前帧分析结果（供 UI 显示）
    @Volatile
    private var latestFrame: FrameResult? = null
    // 节拍计数
    private var beatCounter = 0
    // 每隔多少拍执行一次动作选择（基于音乐感知的"分组"需要 2 拍，可由 UI 调节）
        ...
        speedSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    ...
                    if (currentActionCode in ActionCatalog.SPEEDABLE_CODES) {
                        setSpeedLevel(level)
                    } else {
...
//        stopListScan()
        ...
        audioJob = CoroutineScope(Dispatchers.IO).launch {
            ...
            while (isRecording) {
                ...
                if (read >= 4096) {
                    ...
                    val isBeat = processAudioFrame(audioBytes)
                    if (isBeat) {
    ...
    /**
     * 处理一帧音频数据：调用 [AudioAnalysisEngine] 完成特征提取、BPM 检测、PE 计算。
     *
     * @param audioBytes 4096 字节的 PCM 数据（2048 个 16-bit 采样）
     * @return 本帧是否检测到节拍点（onset）
     */
    private fun processAudioFrame(audioBytes: ByteArray): Boolean {
        if (!lastPlayingState) return false

        val now = System.currentTimeMillis()
        val frame = audioEngine.processFrame(audioBytes, now) ?: return false
        latestFrame = frame

        // 更新 BPM 缓存供 UI 显示
        if (frame.bpm > 0) {
            currentBpmEstimate = frame.bpm
        }

        // 记录 onset 时间戳供 ActionSelector 预测下一拍
        if (frame.isBeat) {
            actionSelector.recordOnset(now)
        }
    ...
    /**
     * 节拍回调：每检测到一个 onset 调用一次。
     *
     * 职责：
     * 1. 递增节拍计数，定期更新 UI 显示（PE/BPM/动作）。
     * 2. 自适应模式下，每 frequencyDivider 拍调用 [ActionSelector.decide] 决策是否切换动作。
     * 3. 若决策为切换，执行 [switchAction] + [setSpeedLevel]。
     */
    private fun onBeatDetected() {
        if (!isDancing) return
        beatCounter++

        // 每 frequencyDivider 拍更新一次 UI 并执行动作选择
        if (beatCounter % frequencyDivider == 0) {
            val frame = latestFrame
            if (frame != null) {
                tvPE.text = "PE: ${String.format("%.2f", frame.pe)}"
                tvBPM.text = "BPM: ${currentBpmEstimate.toInt()}"
                val actionName = ActionCatalog.CODE_TO_DISPLAY_NAME[currentActionCode] ?: currentActionCode
                tvActionStatus.text = "动作: $actionName (档$currentSpeedLevel)"
            }

            if (isAdaptiveMode && frame != null && currentBpmEstimate > 0) {
                appendLog("⚡ 开始动作选择 (节拍 $beatCounter, BPM=${currentBpmEstimate.toInt()})")
                val now = System.currentTimeMillis()
                actionSelector.switchThreshold = switchThreshold
                val decision = actionSelector.decide(
                    bpm = currentBpmEstimate,
                    pe = frame.pe,
                ...
                when (decision) {
                    is SwitchDecision.Switch -> {
                        appendLog("✅ ${decision.reason}: ${decision.newAction.code} 档${decision.newAction.speedLevel} (分差=${String.format("%.3f", decision.scoreDiff)})")
                        switchAction(decision.newAction.code)
                        if (decision.newAction.isSpeedable) {
                            setSpeedLevel(decision.newAction.speedLevel)
                        }
                    }
                    is SwitchDecision.NoSwitch -> {
                        appendLog("⏭️ ${decision.reason}")
                    }
                }
            }
        }
...
//    }
    ...
    private fun setSpeedLevel(targetLevel: Int) {
        ...
        // 同步更新 ActionSelector 的当前动作（档位变化后动作定义也变了）
        ActionCatalog.find(currentActionCode, clampedTarget)?.let { def ->
            actionSelector.setCurrentAction(def, beatCounter)
        }
    }
    ...
    private fun switchAction(actionCode: String) {
        ...
        val displayName = ActionCatalog.CODE_TO_DISPLAY_NAME[actionCode] ?: actionCode
        ...
        if (isConnected) {
            ...
            tvActionStatus.text = "动作: ----"
            Thread.sleep(100)
            sendCommand(ActionCatalog.CODE_TO_COMMAND[actionCode] ?: return)
            Thread.sleep(100)
            sendCommand("FE 55 10 F0 55 FE")
        } else {
        ...
        // 3. 速度滑块启用状态
        if (actionCode in ActionCatalog.SPEEDABLE_CODES) {
            runOnUiThread {
        ...
        // 同步更新 ActionSelector 的当前动作状态（用于驻留时间判断）
        ActionCatalog.find(actionCode, currentSpeedLevel)?.let { def ->
            actionSelector.setCurrentAction(def, beatCounter)
        }
    }
